 ...
 */
export function formatTimeOnly(isoTimeStr) {
    if (typeof isoTimeStr !== "string" || !isoTimeStr.trim())
        return "";
 ...
 */
export function formatSingleEvent(loaded) {
    ...
    if (evt.react && (evt.react.feeling || evt.react.thought || evt.react.action)) {
        const parts = [];
        if (evt.react.feeling)
            parts.push(`感受: ${evt.react.feeling}`);
        if (evt.react.thought)
            parts.push(`思考: ${evt.react.thought}`);
        if (evt.react.action)
            parts.push(`行动: ${evt.react.action}`);
 ...
 */
export function formatLifeContext(events, worldSummary, isDelta = false) {
    if (events.length === 0)
        return "";
    const lines = [];
    ...
    lines.push("");
    lines.push("*你可以将这些生活经历自然融入与用户的对话或日常思考中。如需记录/更新对某事件的感受、思考或行动决策，可调用 `life_react` 工具。*");
    return lines.join("\n");
}
/**
 * Build a concise notice summary for the plugin source attribution.
 */
export function formatNoticeSummary(eventCount, isDelta) {
    if (isDelta) {
