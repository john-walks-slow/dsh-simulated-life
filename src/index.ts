 ...
 */
export async function handleLifePreStep(
	agent: Agent,
	step: number,
	aborted: boolean,
	sessionState: SessionLifeProjectionState | undefined,
	windowHours: number,
	now: number = Date.now()
): Promise<{ message?: UserMessage; nextState?: SessionLifeProjectionState }> {
	if (aborted || step !== 1) {
		return {};
	}
...
export function apply(
	ctx: Context,
	config: { windowHours?: number; enabled?: boolean } = {}
) {
	...
	(ctx as any).tools.register(createLifeReactTool());

	// In-memory projection store mapping sessionId to tracked state (with LRU eviction)
	const sessionMemoryStates = new Map<string, SessionLifeProjectionState>();

	ctx.on(
		"agent/pre-step",
		async (payload: { agent: Agent; messages: UserMessage[]; turn: number; step: number; signal: AbortSignal }, next: () => Promise<PreStepDecision>) => {
			...
			try {
				...
				const result = await handleLifePreStep(
					payload.agent,
					payload.step,
				...
				if (result.nextState) {
					...
					if (sessionMemoryStates.size >= MAX_TRACKED_SESSIONS && !sessionMemoryStates.has(sessionId)) {
						...
						if (oldestKey !== undefined) sessionMemoryStates.delete(oldestKey);
					}
