...
import type { LoadedLifeEvent, InjectedEventRecord, SessionLifeProjectionState, SimulatedEvent } from "./types.js";

/**
 * Generate a deterministic fingerprint hash of an event content and reaction.
 */
export function hashEvent(event: SimulatedEvent): string {
...
export interface DeduplicationDecision {
	...
	newEvents: LoadedLifeEvent[];
	updatedEvents: LoadedLifeEvent[];
	activeEvents: LoadedLifeEvent[];
	nextState: SessionLifeProjectionState;
}
...
export function evaluateDeduplication(
	loadedEvents: LoadedLifeEvent[],
	currentState: SessionLifeProjectionState | undefined,
	...
	const newEvents: LoadedLifeEvent[] = [];
	const updatedEvents: LoadedLifeEvent[] = [];
	const nextInjectedRecords: Record<string, InjectedEventRecord> = {};
	...
	const nextState: SessionLifeProjectionState = {
